...
medical_history = pd.DataFrame({'customer_id': c_ids,
                                ...
                                'has_insurance': [bool(random.getrandbits(1)) for x in range(len(c_ids))]
                             })
...
campaigns = ['influencer', 'wellness_check']
...
sales_scales = pd.DataFrame({'order_id': [fake.isbn13() for x in range(len(c_ids))],
                      ...
                      'customer_id': [random.choice(c_ids) for x in range(len(c_ids))]
                      })
...
sales_mealkits = pd.DataFrame({'order_id': [fake.isbn13() for x in range(n_sales)],
                      'status': random.choices(statuses, weights=(0.800, 0.150, 0.050), k=n_sales),
                      'courier': random.choices(couriers, weights=(0.50, 0.20, 0.30), k=n_sales),
                      'service_level': random.choices(['Standard', 'Expedited'], weights=(0.30, 0.70), k=n_sales),
                      'product_id': [p_id for x in range(n_sales)],
                      'quantity': [random.randint(1, 3) for x in range(n_sales)],
                      'customer_id': [random.choice(c_ids) for x in range(n_sales)]
                      })
